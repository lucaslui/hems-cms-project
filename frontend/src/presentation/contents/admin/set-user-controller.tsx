import hemsApi from '@/domain/api/hems-api'
import regionsApi from '@/domain/api/regions-api'
import usersApi from '@/domain/api/users-api'
import { Hems } from '@/domain/entities/hems'
import { Region } from '@/domain/entities/region'
import { User } from '@/domain/entities/user'
import useForm from '@/presentation/hooks/useForm'
import usePaginator from '@/presentation/hooks/usePaginator'
...
const USER_LIST_INITIAL_STATE: User[] = []
const USER_INITIAL_STATE: User = { id: '', hemsId: '' }
const AVAILABLE_USER_LIST_INITIAL_STATE: User[] = []
...
const SetUserController: React.FC = () => {
  ...
  const handleSelectUserChanged = (event: React.ChangeEvent<HTMLInputElement>): void => {
    ...
    const selectedUser: User = availableUserList.find(availableUser => availableUser.id === setlectedUserId)
  ...
  const handleStartEditUser = (user: User): void => {
    setUser(user)
  }
  const handleStartDeleteUser = (user: User): void => {
  ...
  const deleteUserController = async (user: User): Promise<any> => {
    const updatedUser: User = { ...user, hemsId: '' }
  ...
  const filterUnavailableUsers = (userList: User[]): User[] => {
    return userList.filter(user => user.hemsId)
  }
  const filterAvailableUsers = (userList: User[]): User[] => {
    return userList.filter(user => !user.hemsId)
  }
